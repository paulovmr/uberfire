...
public class PreferenceStorageImpl implements PreferenceStorage {
    ...
    public <T> PreferenceScopedValue<T> readWithScope( final PreferenceScopeResolutionStrategy preferenceScopeResolutionStrategy,
        ...
        for ( PreferenceScope scope : preferenceScopeResolutionStrategy.order() ) {
            ...
            if ( result != null ) {
                return new PreferenceScopedValue<>( result, scope.key() );
            }
        }
    ...
    public Collection<String> allKeys( final List<PreferenceScope> scopes ) {
        ...
        for ( PreferenceScope scope : scopes ) {
            keys.addAll( allKeys( scope ) );
        }

        return keys;
    }

    String buildScopePath( final PreferenceScope preferenceScope ) {
        return "/config/" + preferenceScope.type() + "/" + preferenceScope.key();
    }

    String buildScopedPreferencePath( final PreferenceScope preferenceScope,
                                      final String key ) {
        return buildScopePath( preferenceScope ) + "/" + key + FILE_FORMAT;
    }
}
