...
public class PreferenceStoreImpl implements PreferenceStore {
    ...
    public PreferenceStoreImpl( final PreferenceStorage storage,
                                final PreferenceScopeResolutionStrategy defaultResolutionStrategy ) {
    ...
    public <T> void put( final PreferenceScope scope,
        ...
        for ( final Map.Entry<String, T> entry : valueByKey.entrySet() ) {
            put( scope, entry.getKey(), entry.getValue() );
        }
    }
    ...
    public <T> void putIfAbsent( final PreferenceScopeResolutionStrategy scopeResolutionStrategy,
                                 final String key,
                                 final T value ) {
        if ( !storage.exists( scopeResolutionStrategy.defaultScope(), key ) ) {
            put( key, value );
        }
    }
    ...
    public <T> void putIfAbsent( final PreferenceScope scope,
                                 final Map<String, T> valueByKey ) {
        for ( final Map.Entry<String, T> entry : valueByKey.entrySet() ) {
            putIfAbsent( scope, entry.getKey(), entry.getValue() );
        }
    }
    ...
    public <T> T get( final String key,
                      final T defaultValue ) {
        return get( defaultResolutionStrategy, key, defaultValue );
    }
    ...
    public Map<String, Object> search( final PreferenceScope scope,
        ...
        for ( String key : keys ) {
            map.put( key, storage.read( scope, key ) );
        }
    ...
    public Map<String, Object> search( final PreferenceScopeResolutionStrategy scopeResolutionStrategy,
        ...
        Map<String, Object> map = new HashMap<>();
        for ( String key : keys ) {
            map.put( key, storage.read( scopeResolutionStrategy, key ) );
        }
    ...
    @Override
    public void clear( final List<PreferenceScope> scopes,
                       final String key ) {
