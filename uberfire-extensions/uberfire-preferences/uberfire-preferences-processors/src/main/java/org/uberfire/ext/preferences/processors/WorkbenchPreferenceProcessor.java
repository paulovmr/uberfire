...
public class WorkbenchPreferenceProcessor extends AbstractErrorAbsorbingProcessor {
    ...
    public static final String WORKBENCH_PREFERENCE = "org.uberfire.ext.preferences.client.annotations.WorkbenchPreference";
    ...
    public boolean processWithExceptions( Set<? extends TypeElement> annotations,
        ...
        for ( Element element : roundEnv.getElementsAnnotatedWith( elementUtils.getTypeElement( WORKBENCH_PREFERENCE ) ) ) {
            if ( element.getKind() == ElementKind.CLASS ) {
                WorkbenchPreferenceGeneratedImplGenerator clientImplGenerator = null;
                WorkbenchPreferenceGeneratedImplGenerator storageImplGenerator = null;

                try {
                    clientImplGenerator = new WorkbenchPreferenceGeneratedImplGenerator( GeneratorContext.CLIENT );
                    storageImplGenerator = new WorkbenchPreferenceGeneratedImplGenerator( GeneratorContext.STORAGE );
                } catch (Throwable t) {
                ...
                generate( messager, classElement, packageElement, packageName, className, storageImplGenerator );
                generate( messager, classElement, packageElement, packageName, className, clientImplGenerator );
            }
        }
    ...
    private void generate( final Messager messager,
        ...
        try {
            ...
            if ( callback == null ) {
                writeCode( generator.getTargetPackage(),
